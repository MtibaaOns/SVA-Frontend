...
export class UtilisateurService {
  ...
  public getUtilisateurById(id: number): Observable<Utilisateur[]> {
    ...
    public updateUtilisateur(utilisateur: Utilisateur, id: number, nom: string,prenom: string, adresse: string, email: string, tel: string, mp: string,login: string,role: string): Observable<Utilisateur> {
      // Inclure les paramètres dans le corps de la requête
      return this.http.put<Utilisateur>(`${this.apiServerUrl}/Utilisateurs/update/${id}?nom=${nom}&prenom=${prenom}&email=${email}&tel=${tel}&adresse=${adresse}&mp=${mp}&login=${login}&role=${role}`, utilisateur);
    }
